...
import './MultiSelectSerializedPage.css'

const MultiSelectSerializedPage = () => {
    // const [startDate, setStartDate] = useState(null);
    // const [endDate, setEndDate] = useState(null);
    const [startDate, setStartDate] = useState(new Date()); // Today
    const [endDate, setEndDate] = useState(new Date(new Date().setDate(new Date().getDate() + 6))); // Next 5 days
    // const [planningData, setPlanningData] = useState(defaultData);
    ...
    const toastDisplayed = useRef(false); // ✅ Use useRef to persist across renders

    const fetchPlanningData = async () => {
        if (!startDate || !endDate) {
            toast.error("Please select both Start Date and End Date to fetch planning data.");
            return;
        }
        ...
        const formattedStartDate = new Date(Date.UTC(startDate.getFullYear(), startDate.getMonth(), startDate.getDate()));
        const formattedEndDate = new Date(Date.UTC(endDate.getFullYear(), endDate.getMonth(), endDate.getDate()));
        ...
        try {
            const response = await fetch(
                `http://127.0.0.1:5000/get_planning_date?startDate=${formattedStartDate.toISOString().split('T')[0]}&endDate=${formattedEndDate.toISOString().split('T')[0]}`
            );

            const result = await response.json();
            console.log("Planning Data here:", result);

            if (result.data && result.data.length > 0) {
                const filteredData = result.data.map((part) => {
                    const filteredPlanData = part.planData ? { ...part.planData } : {};

                    // Ensure every date in the selected range is present
                    dateRange.forEach((date) => {
                        if (!filteredPlanData[date]) {
                            filteredPlanData[date] = { quantity: 0, issue_qty: 0 };
                        }
                    });
                ...
                // setMessage("Planning data fetched successfully.");
                ...
                if (!toastDisplayed.current) {
                    toast.success("Planning data fetched successfully.");
                    toastDisplayed.current = true;
                }
            } else {
                setPlanningData([]);
                setMessage("No planning data available for the selected date range.");
                toast.error("No planning data available for the selected date range.");
                setSelectedParts([]);
                setSelectedPartDetails([]);
                setSelectAll(false);
            }
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (staticTableRef.current && scrollableTableRef.current) {
                staticTableRef.current.scrollTop = scrollableTableRef.current.scrollTop;
            }
        };
        ...
        <div style={{ marginTop: '130px' }} className='container'>
                    ...
                    <div className="my-2 text-end">
                        ...
                        {showGenerateSerialButton && (
                            ...
                            >
                                {isLoading ? (
                                    <>
                                        <button className="spinner-border spinner-border-sm m-0" role="status"></button>
           ...
           <div className="scroll_container_table">
            <div style={{ display: "flex", position: "relative" }}>
                {/* Static Table */}
                <div ref={staticTableRef} style={{ flex: "0 0 40%", maxHeight: "300px", overflow: "hidden" }}>
                    <table className="table table-bordered shadow-sm static-table-custom">
                        <thead>
                            <tr>
                                <th>Check</th>
                                <th>Part Number</th>
                        ...
                        <tbody>
                            {filteredPlanningData.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="text-center">No match found</td>
                                </tr>
                            ) : (
                                filteredPlanningData.map((part, rowIndex) => (
                                    <tr key={rowIndex}>
                ...
                {/* Scrollable Table */}
                <div ref={scrollableTableRef} style={{ flex: "0 0 60%", overflow: "auto", maxHeight: "300px" }}>
                    <table className="table table-bordered shadow-sm scrollable-table-custom">
                        <thead>
                            <tr>
                                {planningData.length > 0 && planningData[0].planData &&
                                    Object.keys(planningData[0].planData).map((date, index) => (
                                        <th key={index}>{new Date(date).toLocaleDateString()}</th>
                                    ))}
                                {planningData.length === 0 && <th>No data available</th>}
                            </tr>
                        </thead>
                        <tbody>
                            {filteredPlanningData.length === 0 ? (
                                <tr>
                                    <td colSpan={Object.keys(planningData[0]?.planData || {}).length} className='text-center'>No match found</td>
                                </tr>
                            ) : (
                                filteredPlanningData.map((part, rowIndex) => (
                                    <tr key={rowIndex}>
                                        {part.planData && Object.keys(part.planData).map((date) => (
                                            <td key={`${rowIndex}-${date}`}>
                                                <input
...
export default MultiSelectSerializedPage;
