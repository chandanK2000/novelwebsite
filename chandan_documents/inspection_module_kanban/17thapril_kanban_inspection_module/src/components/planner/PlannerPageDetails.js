...
import PlannerPageIssuedqty from './PlannerPageIssuedqty';


const PlannerPageDetails = () => {
  ...
  const [selectedParts, setSelectedParts] = useState(new Array(3).fill(null)); // Initialize with 5 rows
  ...
  //     // Generate a new row and add it to selectedParts
  //     setSelectedParts((prev) => [...prev, null]);
  //   }
  // };
  ...
  // const handleInputChange = (partNo, date, value) => {
  //     const numericValue = isNaN(value) ? 0 : Number(value);  // Convert to number or default to 0
  //     setPlanData((prevData) => ({
  //         ...prevData,
  //         [partNo]: {
  //             ...prevData[partNo],
  //             [date]: numericValue, // Update with numeric value
  //         },
  //     }));
  // };
  ...
  //     [partId]: {
  //       ...prevData[partId],
  //       [date]: isNaN(value) ? 0 : Number(value), // Convert input to number or default to 0
  //     },
  //   }));
  // };
...
<div className='scroll_container_table'>
      <div className="row my-3">
                    ...
                    <tr key={rowIndex}>
                      <td>
                        {/* <Select
                          ...
                          classNamePrefix="custom" // Add a prefix for easier targeting
                        /> */}        <Select
          key={rowIndex}
        ...
        />
                      ...
                      {/* <td>{selectedPart ? selectedPart.PC : "-"}</td> */}
                      <td>
                        <button
                      ...
                      {dateRange.map((date) => (
                        <td key={date.toISOString()}>
                          <input
                            ...
                            type="number"
                            value={
                              planData[selectedPart?.PART_ID]?.[date.toISOString().split("T")[0]]?.quantity || ""
                            }
                            onChange={(e) =>
                              handleInputChange(
                            ...
                            disabled={
                              !selectedPart || // Disable if no part is selected
                              planData[selectedPart?.PART_ID]?.[date.toISOString().split("T")[0]]?.serialized // Disable if serialized is true
                            }
                            min="0" // Prevent negative input in the HTML input

                          // style={{
                          //   backgroundColor:planData[selectedPart?.PART_ID]?.[date.toISOString().split("T")[0]]?.serialized ? 'gray' : 'transparent',
                          //   cursor: !selectedPart || planData[selectedPart?.PART_ID]?.[date.toISOString().split("T")[0]]?.serialized ? 'not-allowed' : 'pointer',
                          // }}
                          />
                        </td>
                      ))}
      ...
      <PlannerPageIssuedqty/>
    </div>

  );
};

export default PlannerPageDetails;
